...
So, instead of firing a lot of rays from the intersection point where a lot of the only will contribute a minuscule to the Monte Carlo integration sum, we do not multiply the rays by the angle cosine value. Instead, we weigh the rays equally much but make sure we fire rays close to the intersection normal more often and those close to the tangent plane more seldom, in a cosine distribution way. Given a infinite amount samples it will turn out, mathematically, the same but the cosine weighted distribution will converge faster.
...
.Uniform random sampling of hemisphere image
----
Image size:             800x400
...
.Cosine weighted sampling of hemisphere image
----
Image size:             800x400
Amount samples/pixel:   4
Max recursion:          4
