...
func NewSodaCanPepsi(scale float64) *scene.FacetStructure {
...
// NewSodaCanMtnDew
// Normal soda can height is 11.6 cm.
func NewSodaCanMtnDew(scale float64) *scene.FacetStructure {
...
// Normal soda can height is 11.6 cm.
func NewSodaCanFantaOrange(scale float64) *scene.FacetStructure {
...
// Normal soda can height is 11.6 cm.
func NewSodaCanCocaColaClassic(scale float64) *scene.FacetStructure {
	sodaCan := loadSodaCan(filepath.Join(TexturesDir, "sodacan/sodacan_cocacola.jpg"), color.NewColor(0.9, 0.2, 0.2), scale)
	return sodaCan
}

func NewSodaCanCocaColaModern(scale float64) *scene.FacetStructure {
	sodaCan := loadSodaCan(filepath.Join(TexturesDir, "sodacan/sodacan_cocacola_02.png"), color.NewColor(0.9, 0.2, 0.2), scale)
	return sodaCan
}

func NewSodaCanTest(scale float64) *scene.FacetStructure {
	sodaCan := loadSodaCan(filepath.Join(TexturesDir, "test/checkered 360x180 with lines.png"), color.NewColor(0.65, 0.55, 0.2), scale)
	return sodaCan
}

func loadSodaCan(textureFileName string, tabColor *color.Color, scale float64) *scene.FacetStructure {
	sodaCan := wavefront.ReadFacetStructureOrPanic(filepath.Join(ObjFileDir, "sodacan.obj"))
	...
	aluminumMaterialLid := scene.NewMaterial().N("lid").M(0.3, 0.4)
	aluminumMaterialTab := scene.NewMaterial().N("tab").C(tabColor).M(0.3, 0.4)
