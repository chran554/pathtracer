...
func KelvinTemperatureColor2(kelvinTemperature float64) Color {
	...
	if temperature < 66.0 {
		...
		// a = 351.97690566805693, b = 0.114206453784165, c = -40.25366309332127
		// x = (kelvin/100) - 55}
		// red = a + b x + c Log[x]
		x := temperature - 55.0
		red = 351.97690566805693 + 0.114206453784165*x - 40.25366309332127*math.Log(x)
	}

	if temperature < 66.0 {
		// a = -155.25485562709179, b = -0.44596950469579133, c = 104.49216199393888
		// x = (kelvin/100) - 2
		// green = a + b x + c Log[x]
		x := temperature - 2
		green = -155.25485562709179 - 0.44596950469579133*x + 104.49216199393888*math.Log(x)
	} else {
		// a = 325.4494125711974, b = 0.07943456536662342, c = -28.0852963507957
		// x = (kelvin/100) - 50
		// green = a + b x + c Log[x]
		x := temperature - 50.0
		green = 325.4494125711974 + 0.07943456536662342*x - 28.0852963507957*math.Log(x)
	}
	...
	if temperature >= 66.0 {
		...
		if temperature <= 20.0 {
			...
			// a = -254.76935184120902, b = 0.8274096064007395, c = 115.67994401066147
			// x = kelvin/100 - 10
			// blue = a + b x + c Log[x]
			x := temperature - 10
			blue = -254.76935184120902 + 0.8274096064007395*x + 115.67994401066147*math.Log(x)
		}
	}
...
func KelvinTemperatureCCT(c Color) float64 {
	...
	for (maxTemperature - minTemperature) > epsilon {
		...
		if ((testRGB.R - testRGB.B*1.1) / testRGB.G) <= ((c.R - c.B*1.1) / c.G) {
