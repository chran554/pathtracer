...
var amountSamples = 1024 * 8 // * 24 // 1024 * 16
...
func main() {
	...
	lamp2 := obj.NewSquareFacetStructure(obj.SquareTypeXZPlane, nil, false)
	...
	*/
	poolTable := obj.NewSquareFacetStructure(obj.SquareTypeXZPlane, nil, false)
	...
	poolTable.Material = scene.NewMaterial().C(color.White).M(0.015, 0.9).PP(textureTableCloth, &vec3.T{0, 0, 0}, vec3.T{5, 0, 0}, vec3.T{0, 0, 5})
	...
	for animationFrameIndex := 0; animationFrameIndex < amountAnimationFrames; animationFrameIndex++ {
		...
		cameraOrigin := &vec3.T{0, 20, -35}
		aimPoint := &vec3.T{0, 0, 0} // Aim camera at the middle towards the farthest row
		focusPoint := &vec3.T{0, 2.5, -6}
		camera := scene.NewCamera(cameraOrigin, focusPoint, amountSamples, magnification).
			A(apertureSize, nil).
			R(cameraOrigin, aimPoint, false)
