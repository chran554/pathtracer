...
func main() {
	skyDome := scn.NewSphere(&vec3.T{0, 0, 0}, 1000, scn.NewMaterial().
		...
		SP("textures/equirectangular/open_grassfield_sunny_day.jpg", &vec3.T{0, 0, 0}, vec3.T{1, 0, 0}, vec3.T{0, 1, 0})).N("sky dome")
	...
	checkeredMaterial := scn.NewMaterial().PP("textures/floor/7451-diffuse 02.png", &sphere2Origin, *(&vec3.T{1, 0, -1}).Scale(2.0), *(&vec3.T{1, 1, 0}).Scale(2.0))
	...
	treeSphereMaterial := scn.NewMaterial().T(0.0, true, scn.RefractionIndex_WoodSap).M(0.0, 0.0).PP("textures/tree/tree_rings_03.jpg", (&treeSphereProjectionOrigin).Add(&vec3.T{-sphereRadius, 0, -sphereRadius}), *(&vec3.T{1, 0, 0}).Scale(sphereRadius * 2), *(&vec3.T{0, 0, 1}).Scale(sphereRadius * 2))
	...
	camera := scn.NewCamera(origin, focusPoint, amountSamples, magnification).A(1.0, "")
