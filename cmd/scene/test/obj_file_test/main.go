...
var animationName = "obj_file_test"
...
func main() {
	...
	// dice := wavefront.ReadFacetStructureOrPanic(filepath.Join(obj.ObjFileDir, "dice", "dice_lores.obj"))
	dice := wavefront.ReadFacetStructureOrPanic(filepath.Join(obj.ObjFileDir, "dice", "dice_hires.obj"))

	dice.GetFirstMaterialByName("dice_face1").Color = color.NewColor(0.90, 0.60, 0.60)
	dice.GetFirstMaterialByName("dice_face2").Color = color.NewColor(0.60, 0.90, 0.60)
	dice.GetFirstMaterialByName("dice_face3").Color = color.NewColor(0.60, 0.60, 0.90)
	dice.GetFirstMaterialByName("dice_face4").Color = color.NewColor(0.75, 0.75, 0.60)
	dice.GetFirstMaterialByName("dice_face5").Color = color.NewColor(0.60, 0.75, 0.75)
	dice.GetFirstMaterialByName("dice_face6").Color = color.NewColor(0.75, 0.60, 0.75)

	dice.CenterOn(&vec3.Zero)
	dice.RotateY(&vec3.Zero, math.Pi/10.0)
	dice.RotateX(&vec3.Zero, math.Pi/12.0)
	dice.ScaleUniform(&vec3.Zero, 1.0/dice.Bounds.SizeY())
	dice.ScaleUniform(&vec3.Zero, 20.0)
	dice.Translate(&vec3.T{0, -dice.Bounds.Ymin, 0})

	dice.Translate(&vec3.T{0, pillarHeight, 0})

	scn := scene.NewSceneNode().FS(cornellBox, pillar, dice)

	cameraOrigin := dice.Bounds.Center().Add(&vec3.T{-30, 50, -100})
	cameraOrigin.Scale(cameraDistanceFactor)
	focusPoint := dice.Bounds.Center().Add(&vec3.T{0, 0, -dice.Bounds.SizeZ() / 2})
