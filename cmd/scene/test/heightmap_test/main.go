...
func main() {
	...
	animation := scene.NewAnimation("heightmap_test", width, height, magnification, false, true)
	...
	groundDisc := scene.NewDisc(&vec3.Zero, &vec3.UnitY, 10*1000, landscapeMaterial)
	...
	for frameIndex := animationStartIndex; frameIndex <= animationEndIndex; frameIndex++ {
		...
		lamp := scene.NewSphere(&vec3.T{0.75 * size * math.Cos(lightStartAngle+angle), 400, 0.75 * size * math.Sin(lightStartAngle+angle)}, 200.0, scene.NewMaterial().E(color.White, 12, true))
		...
		camera := scene.NewCamera(&cameraOrigin, &viewPoint, amountSamples, magnification).A(apertureSize, nil).F(focusDistance)

		scn := scene.NewSceneNode().FS(landscape).S(lamp, skyDome).D(groundDisc)
