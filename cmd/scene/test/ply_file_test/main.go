...
func main() {
	cornellBox := obj.NewWhiteCornellBox(&vec3.T{200, 300, 300}, true, 3.0) // cm, as units. I.e. a 5x3x5m room
	...
	for degree := minDegree; degree <= maxDegree; degree += 5 {
		...
		cameraOrigin := &vec3.T{0, 0, -100}
		cameraOrigin.Scale(cameraDistanceFactor)
		cameraOrigin.Add(beethoven.Bounds.Center())
